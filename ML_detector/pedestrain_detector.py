# -*- coding: utf-8 -*-
"""
Created on Tue Dec 31 09:06:53 2019
...
4. 使用检测子对测试负样本集进行识别，找到识别错误的hard example：
    hard example是指利用第一次训练的分类器在负样本原图(肯定没有人体)上进行
    行人检测时所有检测到的矩形框，这些矩形框区域很明显都是误报，把这些误报的矩形框保存为图片，
    加入到初始的负样本集合中，重新进行SVM的训练，可显著减少误报。这种方法叫做自举法(Bootstrap)，
    自举法首先使用初始负样本集来训练一个模型，然后收集被这个初始模型错误分类的负样本来形成一个负样本难例集。
    用此负样本难例集训练新的模型，此过程可以重复多次。

5.将hard example提取HOG特征，结合第一步所得到的特征一起训练出模型。

6. 识别：识别有两种方式，如果仅仅使用线性核的话，只需要Hog类自带的setSVMDetector和
    detect（detectMultiScale），如果使用RBF，就需要使用SVM类的predict。
    但predict是判断“是与不是”的，如果要得区域的话，
    使用Hog类的setSVMDetector和detect（detectMultiScale）。
...
#加载不含行人的图片
negImageList = loadImageList(r"G:/Opencv/INRIAPerson/Train", "neg.lst") 
...
image = cv2.imread("images/2020.png")
